 ...
 *
 * Loaded by patcherly.php after patcherly_plugin_header_data() is defined.
 */
 ...
 * matches the connector package release (e.g. 1.44.0). Package URL is the zip asset from that release.
 *
...
function patcherly_fetch_remote_update_info() {
    ...
    foreach ($release['assets'] as $asset) {
        ...
        if ($name === 'patcherly-update.json' && !empty($asset['browser_download_url'])) {
            $json_url = $asset['browser_download_url'];
        }
        if ($name === 'patcherly.zip' && !empty($asset['browser_download_url'])) {
