(function(){
  ...
  function bind(){
    ...
    // Row actions (delete, approve, dismiss) — proxied through PHP for OAuth signing
    ...
    if (tbody) tbody.addEventListener('click', async function(e){
      ...
      if (t && t.classList && t.classList.contains('patcherly-del-one')){
        e.preventDefault();
        tr = t.closest('tr'); id = tr && tr.getAttribute('data-id'); if (!id) return;
        try{
          var j = await doErrorAction('patcherly_error_delete', id);
          if (j && j.success !== false) tr.remove();
        }catch(_){ }
        ...
        tr = t.closest('tr'); id = tr && tr.getAttribute('data-id'); if (!id) return;
        try{
          var jA = await doErrorAction('patcherly_error_approve', id);
          if (jA && jA.success !== false) loadErrors(true);
        }catch(_){ }
        ...
        tr = t.closest('tr'); id = tr && tr.getAttribute('data-id'); if (!id) return;
        try{
          var jD = await doErrorAction('patcherly_error_dismiss', id);
          if (jD && jD.success !== false) loadErrors(true);
        }catch(_){ }
      }
    });
